...
# Öffnen einer Excel-Datei im Format "Robot"
...
# Textdatei mit sicheren Lohnarten für Fall 30
...
def people_classes(x):
    # Zählen und Eintragen der Monate, in denen eine Änderung stattgefunden hat
    person = Person(x)
    local_month = []
    local_lohn = []
    ...
    # Loop durch die Liste mit sicherem Fall 30
    person.fall30 = any(k in lines_fall_30 for k in local_lohn)

    # Loop durch die Liste mit Versicherung und Überprüfung der Prozentsätze
    if person.fall30 is False:
        if any(k in l_fall30_vers for k in local_lohn) and not i[13] == "":
            person.fall30 = True

    # Loop durch die Liste mit Steuerung und Überprüfung der Prozentsätze
    if person.fall30 is False:
...
for i in range(5):
    ...
    # Benennung der Excel-Datei nach dem aktuellen Monat
    if answer == 'Y':
...
def final_report():
    ...
    # Notwendige Legende am Ende des Ergebnisberichts
    ...
    # Überschrift für die Tabelle mit den Personen
    sheet.column_dimensions['A'].width = 20
    sheet['A1'].value = "Zeilenbeschriftungen"
    sheet['N1'].value = "RR A."
    sheet['O1'].value = "Grund"

    # Formatierung der 12 Monate bis zum aktuellen Monat
    for i in range(0, 12):
    ...
    # Formatierung der erhaltenen Daten/Ergebnisse
    for i in range(len(all_the_people)):
